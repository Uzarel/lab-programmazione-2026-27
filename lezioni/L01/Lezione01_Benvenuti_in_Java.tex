% ============================================================================
%  Laboratorio di Programmazione -- Lezione 1 (25 settembre 2026)
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item JDK e versioni; installazione su Windows, macOS e Linux
    \end{itemize}
...
\vspace{0.2cm}
\small Martedì, 2h, fino al 15 dicembre (l'8 dicembre è festivo), più alcuni laboratori di sola esercitazione. Il programma completo, con date e contenuti di ogni lezione, sarà pubblicato sul team Microsoft Teams del corso (seguirà comunicazione).
\end{frame}
...
% ========================================================================
\begin{frame}{JDK e versioni: che cosa installare}
\begin{columns}[T]
\column{0.5\textwidth}
\textbf{JDK -- Java Development Kit}
\begin{itemize}\itemsep=2pt
    \item tutto ciò che serve per \textbf{sviluppare} ed \textbf{eseguire}: il compilatore \texttt{javac}, la JVM \texttt{java}, gli strumenti (\texttt{jshell}, \texttt{javadoc}, \texttt{jar}\ldots) e la libreria standard
    \item è l'unica cosa da installare: contiene tutto
\end{itemize}
...
% ========================================================================
\begin{frame}[fragile]{Installare il JDK 25 (Eclipse Temurin)}
\small
\textbf{Windows} -- da \url{https://adoptium.net} scaricare \emph{Temurin 25 (LTS)} come \texttt{.msi}; nell'installer scegliere \emph{Custom} e attivare \textbf{Set JAVA\_HOME} e \textbf{Add to PATH}.

\vspace{0.15cm}
\begin{columns}[T]
\column{0.5\textwidth}
\textbf{macOS} -- prima il chip: menu Apple \textrightarrow{} \emph{Informazioni su questo Mac}: \emph{Apple M1--M4} \textrightarrow{} \textbf{aarch64}, \emph{Intel} \textrightarrow{} \textbf{x64}. Poi da \url{https://adoptium.net} il \texttt{.pkg} per il proprio chip (doppio clic e avanti), oppure con Homebrew:
\begin{lstlisting}[style=shell,basicstyle=\ttfamily\scriptsize\color{white}]
$ brew install --cask temurin@25
\end{lstlisting}

\column{0.5\textwidth}
\textbf{Linux} -- il modo più semplice, uguale per tutte le distribuzioni, è \textbf{SDKMAN} (non richiede root):
\begin{lstlisting}[style=shell,basicstyle=\ttfamily\scriptsize\color{white}]
$ curl -s "https://get.sdkman.io" | bash
$ source ~/.sdkman/bin/sdkman-init.sh
$ sdk install java 25-tem
\end{lstlisting}
In alternativa il pacchetto della distribuzione, se già alla 25: \texttt{sudo apt install openjdk-25-jdk} (Ubuntu/Debian), \texttt{sudo dnf install java-25-openjdk-devel} (Fedora).
\end{columns}

\vspace{0.15cm}
\begin{block}{In ogni caso}
Serve il \textbf{JDK} completo, versione \textbf{25}. Dopo l'installazione aprite un \textbf{nuovo} terminale: quello già aperto non vede i programmi appena installati.
\end{block}
\end{frame}
...
\begin{itemize}\itemsep=0pt
    \item avete installato solo la runtime: serve il JDK completo
\end{itemize}
\textbf{Se compare una versione diversa da 25}
\begin{itemize}\itemsep=0pt
    \item c'è un altro Java installato che viene trovato per primo: su Windows controllate \texttt{JAVA\_HOME} e l'ordine nel \texttt{PATH}; su macOS \texttt{/usr/libexec/java\_home -V} elenca i JDK presenti
\end{itemize}
\column{0.5\textwidth}
\textbf{Se nessuno dei due funziona}
\begin{itemize}\itemsep=0pt
    \item JDK non installato, oppure non è nel \texttt{PATH}
    \item avete riaperto il terminale dopo l'installazione?
\end{itemize}
\textbf{IDE}
\begin{itemize}\itemsep=0pt
...
\vspace{0.3cm}
\textbf{Prossima lezione}: il sistema dei tipi -- primitivi, costanti, riferimenti e \texttt{null} -- e le stringhe (\texttt{String}, \texttt{StringBuilder}).
\end{frame}

% ========================================================================
\begin{frame}{Homework 1 (da svolgere entro la prossima lezione)}
\begin{enumerate}
...
Non è obbligatorio, ma fortemente consigliato: alla prossima lezione chiederò a qualcuno di aprire il proprio codice e presentarlo alla classe, e lo discutiamo insieme. Tenete i file \texttt{.java} in una cartella \texttt{hw1/} del progetto VS Code; ogni file deve compilare ed eseguire. Nomi di classi, metodi e variabili secondo le convenzioni viste oggi.
\end{block}
\end{frame}
